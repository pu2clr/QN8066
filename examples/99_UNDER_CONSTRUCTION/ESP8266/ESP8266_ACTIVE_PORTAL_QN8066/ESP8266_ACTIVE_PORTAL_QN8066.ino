/*
  ESP8266 Dev Modeule version.

1. This application is very similar to the '07_WEB_WIFI_ESP8266_QN8066' application, with one subtle difference: 
   the ESP8266 itself will provide its own internet network (Access Point), allowing you to configure the FM 
   Transmitter by connecting your device (phone or computer) directly to the ESP8266. This eliminates the need 
...
2. This sketch uses HTML form to get and process commands from external browser applications to configure the 
   FM transmitter (see the Python example `ESP8266_qn8066.py`).
3. It also utilizes the internal Real Time Clock (RTC) of the ESP8266 to send RDS messages with
   this information.
...
7. This sketch was compiled and tested on the ESP8266 Der Module. If you are using a different ESP8266 model, 
...
8. In the Arduino IDE, during application execution, check the Serial Monitor for the IP address 
   assigned to the ESP8266 by your router's DHCP service. 
9. RDS message updates such as PTY, PS, RT, and Time are not executed immediately. This may depend on 
  ...
  ESP8266 Internal RTC: https://github.com/fbiego/ESP8266Time
...
// #include <DNSServer.h>
#include <time.h>    // Using internal RTC of ESP8266 
#include <QN8066.h>
...
const char *password = "ESP8266qn8066";


IPAddress local_IP(10, 0, 0, 1);        // Endereço IP fixo
IPAddress gateway(10, 0, 0, 1);         // Gateway (o próprio ESP8266)
...
// Converts String Date format (yyyy/mm/dd hh:mm) to ESP8266 RTC and update RDS Date and Time message. 
void setRTC(String datetime) {
  ...
  settimeofday(&now, NULL); // Defines ESP8266 RTC time
  rdsSendDateTime();
  Serial.println("Internal ESP8266 RTC updated!");
}
...
void setup() {
  ...
  // The line below may be necessary to setup I2C pins on ESP8266
