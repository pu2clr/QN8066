...
const uint8_t app_id = 33;  // Useful to check the EEPROM content before processing useful data
...
void showStatus() {
  ...
  if ( (millis() - timeOnTheAr) > 60000) { 
    countTime++;
    timeOnTheAr = millis();
  }
  sprintf(str, "On the Air: %dmin", countTime);
  lcd.setCursor(0, 3);
  lcd.print(str);
  // END DEBUG
