...
def send_to_esp32(field, value):
    try:
        ...
        esp32_ip = '127.0.0.1'  # ESP32 IP - Check it in the Arduino IDE Serial Monitor (console)
...
def send_rds_ps():
    ...
    send_to_esp32("rds_ps", rds_ps[:8].ljust(8))

def send_rds_rt():
    rds_rt = rds_rt_var.get()
    send_to_esp32("rds_rt", rds_rt[:32].ljust(32))
